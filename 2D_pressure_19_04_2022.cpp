...
double (*d_minus)(double x);
double (*d_plus)(double x);
...
int main()
{
    ...
    for (size_t i = 0; i < x.size(); i++)
    {
        if (center[i].x > 0.0)
        {
            ...
            if (center[i].x == x_center_max)
            {
                ...
                u_n_plus[i] = (P_G - x[i]) * (j_plus(d_plus(d_max) / H_up, R_up_plus / H_up, 1.0, L_up_plus / H_up));
                u_n_minus[i] = (P_G - x[i]) * (j_minus(d_minus(d_max) / H_up, R_up_minus / H_up, 1.0, L_up_minus / H_up));
                j[i] = (j_plus(d_plus(d_max) / H_up, R_up_plus / H_up, 1.0, L_up_plus / H_up) + j_minus(d_minus(d_max) / H_up, R_up_minus / H_up, 1.0, L_up_minus / H_up));
                u_n[i] = (P_G - x[i]) * (j_plus(d_plus(d_max) / H_up, R_up_plus / H_up, 1.0, L_up_plus / H_up) + j_minus(d_minus(d_max) / H_up, R_up_minus / H_up, 1.0, L_up_minus / H_up));
                d_max = -999999.0;
                d_min = -999999.0;
            }
            else 
        ...
        else if (center[i].x < 0.0)
        {
            ...
            if (center[i].x == x_center_min)
            {
                ...
                u_n_plus[i] = (P_G - x[i]) * (j_plus(abs(d_plus(d_min) / H_down), R_down_plus / H_down, 1.0, L_down_plus / H_down));
                u_n_minus[i] = (P_G - x[i]) * (j_minus(abs(d_minus(d_min) / H_down), R_down_minus / H_down, 1.0, L_down_minus / H_down));
                j[i] = (j_plus(abs(d_plus(d_min) / H_down), R_down_plus / H_down, 1.0, L_down_plus / H_down) + j_minus(abs(d_minus(d_min) / H_down), R_down_minus / H_down, 1.0, L_down_minus / H_down));
                u_n[i] = (P_G - x[i]) * (j_plus(abs(d_plus(d_min) / H_down), R_down_plus / H_down, 1.0, L_down_plus / H_down) + j_minus(abs(d_minus(d_min) / H_down), R_down_minus / H_down, 1.0, L_down_minus / H_down));
                d_max = -999999.0;
                d_min = -999999.0;
            }
            else
    ...
    //vivod_xyz(grid, center, u_n_minus, "un_minus_value");
